...
남자-> 1, 여자 -> 0일 때 and 연산시 true가 나오는 경우를 두 집단 길이 차이에서 빼면 곧 포옹한 횟수가 된다.

2. 자릿수가 올라갈 때마다 연산을 하는 것이 자릿수가 올라갈 때마다 짧은 수를 올리는 곱셈과 비슷함을 느낄 수 있다.
두 배열 중 한 쪽을 거꾸로 뒤집고 자릿수가 10이 아닌 무한이라고 가정한 후 그룹 인원이 k명일 때 실제 계산을 해볼 경우 
2^k+1승 자리부터 각 자리수가 0이면 남자끼리 만난 경우가 없다는 뜻이다.
즉 두 수의 곱셈에서 각 자리수가 0인지를 판별하면 포옹한 경우를 모두 구할 수 있다.

3. 카라츠바 곱셈은 각 자릿수를 연산 과정에서 올리지 않고 보존한다. 이를 활용하면 빠르게 결과를 구할 수 있다.
이걸 이용해서 각 자리에서 나오는 0을 세면 1의 방법을 이용해 답을 구할 수 있다.
